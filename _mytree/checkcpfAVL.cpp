...
void avl::imprimeAutorizadosAux(noh* atual, bool impar) {
        ...
        percorrePreOrdemAux(atual, impar);
}
...
bool avl::verificaAutorizado(tipoChave chave, int dia) {
    if(buscaAux(chave) == NULL)
        cout << "Erro na verificacao: chave inexistente!" << endl;
         
    else if(chave % 2  == dia % 2 )
        return true;
    return false;    
}

// utiliza o nó atual e seu nível na árvore (para facilitar visualização)
void avl::percorrePreOrdemAux(noh* atual, int nivel) {
  if(atual != NULL){
    percorrePreOrdemAux(atual->left, nivel+1);
    if(nivel % 2 != 0){
        cout << atual->elemento << endl;
    }
    percorrePreOrdemAux(atual->right, nivel+1);
  }
}
